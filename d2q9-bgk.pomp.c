...
#pragma omp parallel                 private(index) POMP2_DLIST_00001 firstprivate(pomp2_old_task) if(pomp2_if) num_threads(pomp2_num_threads)
  ...
  /* loop over the cells in the grid */
{
  ...
  POMP2_Parallel_fork(&pomp2_region_2, pomp2_if, pomp2_num_threads, &pomp2_old_task, pomp2_ctc_2 );
    #pragma omp parallel                 private(index) POMP2_DLIST_00002 firstprivate(pomp2_old_task) if(pomp2_if) num_threads(pomp2_num_threads)
...
#pragma omp parallel private(index, cell, speed) POMP2_DLIST_00003 firstprivate(pomp2_old_task) if(pomp2_if) num_threads(pomp2_num_threads)
...
//          double cell_speed[NSPEEDS];
        for (int kk = 0; kk < NSPEEDS; kk++)
        {
            cells[index].speeds[kk] = *(speed+kk) + params.omega * (d_equ[kk] - *(speed+kk));
        }
//          for (int kk = 0; kk < NSPEEDS; kk++) {
...
#pragma omp parallel                 reduction(+:tot_u, tot_cells) POMP2_DLIST_00005 firstprivate(pomp2_old_task) if(pomp2_if) num_threads(pomp2_num_threads)
...
#pragma omp parallel                 reduction(+:total) POMP2_DLIST_00006 firstprivate(pomp2_old_task) if(pomp2_if) num_threads(pomp2_num_threads)
